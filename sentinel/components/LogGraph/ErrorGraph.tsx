...
const ErrorGraph = ({ logs }: ErrorGraphProps) => {
    ...
    // ---------- Aggregate logs ----------
    const { buckets, seriesByCode, maxCount, allCodes, totalCodes, codeTotals } = useMemo(() => {
        ...
        for (const log of logs || []) {
            ...
            const ts =
                getLogTimestamp(log) || (log as any).raw?.timestamp || (log as any).raw?.time;
            if (!ts) continue;
        ...
        for (const code of codesArray) {
            ...
            const jitterFraction =
                codesArray.length > 1 ? (codeIndex - centerIndex) / (codesArray.length + 1) : 0;
    ...
    useEffect(() => {
        ...
        const bisectDate = d3.bisector<Bucket, Date>(
            (b, d) => b.date.getTime() - d.getTime()
        ).center;
        ...
        const overlay = g
            ...
            .on("mousemove", (event) => {
                ...
                const codesInBucket = Array.from(bucket.perCode.keys()).sort((a, b) => a - b);
                for (const code of codesInBucket) {
                    ...
                    const ids = logsHere
                        ...
                        .map((l) => l.id)
                        .join(", ");
                    const more = logsHere.length > 3 ? `, +${logsHere.length - 3} more` : "";
            ...
            <div ref={containerRef} className="relative flex-1 min-h-0 overflow-x-auto">
                ...
                {noCodes && (
                    <div className="absolute inset-0 flex items-center justify-center text-neutral-400">
                        No HTTP status codes found. Load an HTTP dataset or entries with app type
                        "HTTP-App".
                    </div>
                )}
            </div>
